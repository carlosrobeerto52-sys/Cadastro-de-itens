...
app.get<{ Params: ParamsId }>('/itens/:id', async (request, reply) => {
  ...
  if (!item) {
    return reply.status(404).send({ mensagem: 'Item não encontrado no estoque.' });
  }
...
app.put<{ Params: ParamsId; Body: Partial<ItemBody> }>('/itens/:id', async (request, reply) => {
  ...
  const itemIndex = estoque.findIndex((i) => i.id === Number(id));
  ...
  return reply.status(200).send(estoque[itemIndex]);
});
